...
@interface BankCardListView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
-(void)selectExpressClicked:(id)sender{
    if (self.bankModel) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(selectPayBank:)]) {
            [self.delegate selectPayBank:self.bankModel];
        }
    }
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    self.bankModel = [self.bankArr objectAtIndex:row];
}

-(void)setBankArr:(NSArray *)bankArr{
    _bankArr = bankArr;
    self.bankModel = [bankArr firstObject];
    [self.pickerView reloadComponent:0];
}

-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"bankListCell" forIndexPath:indexPath];
    if (!cell) {
        cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"bankListCell"];
    }
    BankModel *model = [self.bankArr objectAtIndex:indexPath.row];
    cell.textLabel.text = model.name_zh;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BankModel *model = [self.bankArr objectAtIndex:indexPath.row];
    if (self.delegate && [self.delegate respondsToSelector:@selector(selectPayBank:)]) {
        [self.delegate selectPayBank:model];
    }
    [self removeFromSuperview];
}
