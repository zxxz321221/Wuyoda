...
@interface NewLoginViewController ()<ASAuthorizationControllerDelegate,ASAuthorizationControllerPresentationContextProviding,GraphCodeViewDelegate>
...
- (void)viewDidLoad {
    ...
    nav.titleLab.text = @"登录";
//    if ([self.type isEqualToString:@"register"]) {
//        nav.backBtn.hidden = NO;
//    }else{
...
//    }
    ...
    [self.phoneBtn setTitle:@"手机登录" forState:UIControlStateNormal];
    ...
    [self.emailBtn setTitle:@"账号登录" forState:UIControlStateNormal];
    ...
    self.emailTextF.placeholder = @"请输入电子邮箱/手机号码";
    ...
    [self.accountCodeTextF mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_offset(kWidth(-120));
        make.height.mas_offset(kWidth(35));
    }];
    ...
    [self.registerBtn setTitle:@"没有账号，注册" forState:UIControlStateNormal];
    [self.registerBtn setTitleColor:[ColorManager MainColor] forState:UIControlStateNormal];
    ...
    [loginBGV addSubview:self.registerBtn];
    ...
    [loginBtn setBackgroundImage:kGetImage(@"login_按钮") forState:UIControlStateNormal];
    [loginBtn addTarget:self action:@selector(loginCkicked:) forControlEvents:UIControlEventTouchUpInside];
    [loginBGV addSubview:loginBtn];
...
-(void)loginCkicked:(UIButton *)sender{
...
-(void)checkWeiXinAccount:(NSNotification *)notification{
    ...
    [FJNetTool postWithParams:dic url:Login_weixinLogin loading:YES success:^(id responseObject) {
        ...
        if ([baseModel.code isEqualToString:CODE0]) {
            ...
            if (!userModel.member_pass.length && ![LoginUsersModel userSetting:userModel]) {
...
-(void)passwordLogin{
 ...
 //   BOOL result2 = [_str isEqualToString:_codeTextField.text];
    ...
    NSDictionary *dic =@{@"phone":self.emailTextF.text,@"pwd":self.passwordTextF.text,@"verify":self.imgCodeStr,@"code_id":[keyDic objectForKey:KEY_PASSWORD],@"encry":md5Str};
