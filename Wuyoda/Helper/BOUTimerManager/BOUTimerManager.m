...
@property (nonatomic, nullable, strong) dispatch_source_t teacherregisterTimer;// 老师注册界面倒计时timer
...
- (void)timerCountDownWithType:(BOUCountDownType)countDownType {
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        if (interval <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                }else if ([_timer isEqual:self.teacherregisterTimer]) {
                    [[NSNotificationCenter defaultCenter] postNotificationName:kteacherRegisterCountDownCompletedNotification object:@(interval)];
                }
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ([_timer isEqual:self.loginTimer]) {
                    ...
                    [[NSNotificationCenter defaultCenter] postNotificationName:kteacherRegisterCountDownExecutingNotification object:@(interval)];
                }
    ...
    }else if (self.countDonwnType == BOUCountDownTypeteacherRegister) {
        self.teacherregisterTimer = _timer;
    }
...
- (void)cancelTimerWithType:(BOUCountDownType)countDownType {
    switch (countDownType) {
        ...
        case BOUCountDownTypeFindPassword:
            ...
            break;
        case BOUCountDownTypeteacherRegister:
            if (self.teacherregisterTimer) {
                dispatch_source_cancel(self.teacherregisterTimer);
            }
