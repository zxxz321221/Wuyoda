...
#pragma mark - 获取阿里云身份识别
+ (NSURLSessionTask *)IdentityPOST:(NSString *)URL
                parameters:(id)parameters
                   success:(PPHttpRequestSuccess)success
                   failure:(PPHttpRequestFailed)failure {
    return [self IdentityPOST:URL parameters:parameters responseCache:nil success:success failure:failure];
}
...
//    NSString *tokenStr = [RegisterModel getUserInfoModel].user_token?:@"";
//    DLog(@"token:%@",tokenStr);
...
+ (NSURLSessionTask *)IdentityPOST:(NSString *)URL
    ...
    //NSString *tokenStr = [RegisterModel getUserInfoModel].token?:@"";
    //DLog(@"token:%@",tokenStr);
    //[_sessionManager.requestSerializer setValue:tokenStr forHTTPHeaderField:@"token"];
//    [_sessionManager.requestSerializer setValue:[NSString stringWithFormat:@"APPCODE %@" , AliAppCode] forHTTPHeaderField:@"Authorization"];
    //读取缓存
    responseCache!=nil ? responseCache([PPNetworkCache httpCacheForURL:URL parameters:parameters]) : nil;
...
//    [_sessionManager.requestSerializer setValue:[RegisterModel getUserInfoModel].user_token?:@"" forHTTPHeaderField:@"token"];
...
//        [formData appendPartWithFileURL:[NSURL URLWithString:filePath] name:name error:&error];
        ...
        [formData appendPartWithFileURL:[NSURL fileURLWithPath:filePath] name:name fileName:[NSString stringWithFormat:@"%@.mp4",fileName] mimeType:@"application/octet-stream" error:nil];
